...
class DBSynchronizer(object):
    ...
    @classmethod
    def synchronizeRecord(objclass, recordname, force):
        ri = getRecordsInfo()
        rinfo = ri.get(recordname, None)
        if not rinfo: return ErrorResponse("Record not found: %s " % recordname)
        if not rinfo["Persistent"]: return True
        if objclass.tableExists(recordname): 
            return objclass.updateTable(rinfo, force)
        else:
            return objclass.createTable(rinfo)
    ...
    def updateTable(objclass, rinfo, force):
        ...
        query.sql += ",\n".join(sqllines)
        query.sql += "\n,CONVERT TO CHARACTER SET utf8 COLLATE utf8_general_cs"
