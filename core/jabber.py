...
class Client(Connection):
    ...
    def __init__(self, host, port=5222, debug=[], log=False,
        ...
        self._reg_info = {}
        self._reg_agent = ''
        #como estaba el tema de registrar no funcionaba, para esto le agrego el parche siguiente
        self._reg_tags =["username","name","password","email"]
        self._chreg_tags=["username","password"] # para cambiar el password
        self._auth_flag = False
        self._auth_username=None

    def resetRegTags(self):
        self._reg_tags =["username","name","password","email"]

    def addToRegTags(self,regTagName):
        """ To extend the register function """
        self._reg_tags.insert(regTagName)
    ...
    def sendRegInfo(self, agent=None):
        ...
        for info in self._reg_tags:
                q.insertTag(info).putData(self._reg_info[info]) 
        return self.SendAndWaitForResponse(reg_iq)

    def changePassword(self,passwordText,agent=None):
        if self._auth_username:
            d = {}
            d["username"]=self._auth_username
            d["password"]=passwordText
            return self.changeRegInfo(d,agent)

    # to change registration info
    def changeRegInfo(self,newInfo,agent=None):
        if not self._auth_flag: return
        """Sends changes into regitration info to the server (pass,...).
        Example: SomeAuthClient.changeRegInfo(password='myNewPass' """
        if agent: agent = agent + '.'
        if agent is None: agent = ''
        reg_iq = Iq(to = agent + self._host, type='set')
        q = reg_iq.setQuery(NS_REGISTER)
        for info in newInfo.keys():
            if (info in self._chreg_tags):
                q.insertTag(info).putData(newInfo[info]) 
        return self.SendAndWaitForResponse(reg_iq)
